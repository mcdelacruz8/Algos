...
function isPalindrome(queue) {
  ...
  //generate a temp queue and a new array
  var tempQueue = new Queue();
  var palindromeCheck = []
  //put each data to array
  while (!queue.isEmpty()) {
    var tempNode = queue.dequeue();
    palindromeCheck.push(tempNode.data);
    tempQueue.enqueue(tempNode);
  }
  //bring the original queue back
  while (!tempQueue.isEmpty()) {
    queue.enqueue(tempQueue.dequeue());
  }
  ...
  for (let i = 0; i < length / 2; i++) {
    //if found one pair does no equal, return false
    if (palindromeCheck[i] !== palindromeCheck[length - 1 - i]) {
      return false;
    }
  }
  //if all pair are equal, return true
  return true;
}
